...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     清空指定串口的接收和发送 FIFO 缓冲区
// 参数说明     uart_n        串口模块号(UART_0, UART_1, UART_2, UART_3)
// 返回参数     void
// 使用示例     uart_clear_buffer(UART_1);
//-------------------------------------------------------------------------------------------------------------------
void uart_clear_buffer(uart_index_enum uart_n)
{
    IfxAsclin_Asc* uart_handle;
    uart_handle = uart_get_handle(uart_n);
    ...
    while (IfxAsclin_getRxFifoFillLevel(uart_handle->asclin) > 0)
    {
        ...
        IfxAsclin_read8(uart_handle->asclin, (uint8 *)&dummy, 1);
    }

    // 清空发送 FIFO（硬件不支持直接清空，只能等待它发完）
